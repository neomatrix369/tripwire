...
| Full scanner coverage | + Snyk, Tessl, Cisco AI Defense keys (missing Snyk/Cisco key → `skipped_missing_credential`; missing `TESSL_TOKEN` or `TESSL_WORKSPACE` → Review is `needs_setup`, Lint still runs) |
