...
3. **Operator receives an accountable partial-batch result** `@contract-shape:bounded-change`
...
4. **Operator receives an accountable successful-batch result** `@contract-shape:bounded-change`
...
5. **Operator receives usable invalid-input guidance** `@contract-shape:bounded-change`
...
6. **Operator receives usable target-input guidance** `@contract-shape:bounded-change`
...
7. **Installed command remains reachable** `@contract-shape:unbounded-preservation` `@walking_skeleton`
...
**Test inventory (≤7 acceptance tests):** schema error; dry preview; partial batch; successful two-target batch; invalid-input table; target-input error; installed-command walking skeleton.

## Before-Checks [GATE]

- [ ] Branch `slice/18-cli-operator-evidence` created from current `main`
- [ ] `(cd cli && npm run test:coverage)` output and thresholds are recorded in `docs/plan/gate-evidence/slice-18.json`
- [ ] `rg -n "runScan" cli/test` identifies the public-result fixture recorded in `docs/plan/gate-evidence/slice-18.json`
...
- [ ] `docs/plan/gate-evidence/slice-18.json.review` records `acceptance: APPROVED` and `implementation: APPROVED`
- [ ] `docs/plan/gate-evidence/slice-18.json` records commands, coverage, complexity, reviewer verdicts, and `PASS`
- [ ] `docs/plan/gate-evidence/slice-18.json.documentation_audit` records either the changed command-help path and `rg` result or `N/A` with reason
