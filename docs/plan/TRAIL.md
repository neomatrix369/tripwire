...
- **Canonical plan path**: `docs/plan/` (public). Product SoT remains gitignored `private source` — do not fork parallel plan trees. Enhanced-flow-planner context pack: `private source` + `01_demo_video/00-tripwire-demo-script.md` (not `02_prototypes/import-stash/`).
...
| 4 | [`slices/04-D-operator-onboarding/`](slices/04-D-operator-onboarding/) | **D — Role-based onboarding** | **17** | ✅ |
