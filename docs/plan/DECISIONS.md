...
| 2026-08-17 | slice-40 | Scanner Complexity Decomposition — Must, highest priority | Decomposes `run_snyk` (CC=28, D) and `run_cisco_mcp_scanner` (CC=23, D) in `sandbox/scanners.py` to rank C. Originally filed as a skills_scanner cross-project stub (slice 35); reframed 2026-08-17 as a first-party Tripwire refactor with no external dependencies. MoSCoW Must; next slice to execute. Spec: `docs/plan/slices/08-H-frontline-agent-hooks/slice-40-scan-complexity-decomposition.md`. |
