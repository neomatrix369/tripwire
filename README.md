...
Operator-visible Workflow honesty (judge/coverage narration, parent target meta,
multi-select for review, role-labelled models per step/target) is **PLANNED** as
Wave R slice 75 — see [docs/STATUS.md](docs/STATUS.md); do not treat current
Workflow chrome as a complete live judge pipeline yet.
...
| Review | Heatmap, drawers, pathway strips, Escalated / SIE-only filters; Run→Report chrome on **Workflow** tab (slice 75 PLANNED: operator-visible narration, parent meta, multi-select, role-labelled models) | Dashboard (Live or Mock) | [reading-router-results](docs/user-guide/reading-router-results.md) · [screenshots](docs/screenshots/README.md) |
