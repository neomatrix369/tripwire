...
Operator-visible judge/coverage narration (panel-off honesty, process CTA) is **IMPLEMENTED** (unit) on Wave R slice 75 — see [docs/STATUS.md](docs/STATUS.md). L→R soft-amend extras (parent target meta, multi-select, role-labelled models) remain **open**; do not treat the Workflow tab as fully complete until those land and `/verify-slice` closes.
...
| Review | Heatmap, drawers, pathway strips, Escalated / SIE-only filters; Run→Report chrome on **Workflow** tab (slice 75: core narration IMPLEMENTED unit; L→R soft-amend still open) | Dashboard (Live or Mock) | [reading-router-results](docs/user-guide/reading-router-results.md) · [screenshots](docs/screenshots/README.md) |
